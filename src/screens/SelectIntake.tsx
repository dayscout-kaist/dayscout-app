...
export const SelectIntake: React.FC<RootStackScreenProps<"SelectIntake">> = ({
  ...
  const [nutritionInfo, setNutritionInfo] = useState<FoodData>({
    식품명: "",
    제조사명: "",
    단위중량: 1,
    식품중량: "",
  ...
  const calculateNutrition = () => {
    ...
    return {
      ...
      선택중량: `${(Number(quantity) * currentUnitWeight).toFixed(
        2)}${params.foodInfo.content.primaryUnit}`,
      단백질: params.foodInfo.content.nutrients.protein
        ? params.foodInfo.content.nutrients.protein * factor
        : null,
      지방: params.foodInfo.content.nutrients.fat
        ? params.foodInfo.content.nutrients.fat * factor
        : null,
      탄수화물: params.foodInfo.content.nutrients.carbohydrate
        ? params.foodInfo.content.nutrients.carbohydrate * factor
        : null,
      당류: params.foodInfo.content.nutrients.sugar
        ? params.foodInfo.content.nutrients.sugar * factor
        : null,
    };
  };
  ...
  const essentialNutrients = [
    {
      name: "단백질",
      val: nutritionInfo.단백질 ? `${nutritionInfo.단백질.toFixed(2)}g` : "해당없음",
    },
    {
      name: "지방",
      val: nutritionInfo.지방 ? `${nutritionInfo.지방.toFixed(2)}g` : "해당없음",
    },
    {
      name: "탄수화물",
      val: nutritionInfo.탄수화물 ? `${nutritionInfo.탄수화물.toFixed(2)}g` : "해당없음",
    },
    {
      name: "당류",
      val: nutritionInfo.당류 ? `${nutritionInfo.당류.toFixed(2)}g` : "해당없음",
    },
  ...
  return (
    <View style={[bg.white, { flex: 1 }]}>
      ...
      <View style={{ flexDirection: "row" }}>
        {unit && (unit === "그램" || unit === "스푼") ?
          <TextInput
            ...
            textAlign="center"
          /> :
          <Picker style={{ flex: 0.5 }} selectedValue={quantity}
                  onValueChange={(itemValue: React.SetStateAction<string | number>) => setQuantity(
                    itemValue)}>
            {[
              { label: "1/4", value: 1 / 4 },
              { label: "1/3", value: 1 / 3 },
              ...
              ...Array.from({
                length: Math.floor(
                  params.foodInfo.content.totalWeight / nutritionInfo.단위중량 * 3),
        ...
        <Picker
          style={{ flex: 0.5 }}
          selectedValue={unit}
          onValueChange={(itemValue: React.SetStateAction<string>) => setUnit(
            itemValue)}
        >
      ...
      <Text style={[padding.horizontal(16), text.body]}>
        {(unit === "컵" || unit === "공기" || unit === "스푼")
          ? "주의: 음식 종류에 따라 g이 다를 수 있습니다"
          : ""}
      </Text>
